...
{
	//perspective(See all of model based on where you look)
	float fRatio = static_cast<float>(width) / height;
	static float fov = 45.0f;
	static float fNear = 11.001f;
	static float fFar = 12.0f;
	m_m4Projection = glm::perspective(glm::radians(45.0f), fRatio, fNear, fFar);
	//Orthographic(See 2D shape of model)
